...
export default function Checklist() {
  ...
  const renderCkGrouped = (items) => {
    ...
    const keys = Object.keys(grouped)
      .filter(g => !ckHiddenPartnerGroups.includes(g) || ckGroups.includes(g))
    ...
    return keys.map((group, i) => {
      ...
      const isOwnGroup = ckGroups.includes(group)
      const hasPartnerItems = grouped[group].some(t => t.user_id !== user?.id)
      const isHideable = !isOwnGroup && hasPartnerItems
      return (
          ...
          {grouped[group].map((item, itemIdx) => {
            ...
            <li key={item.id} className={`border rounded-[8px] px-2 py-[6px] flex items-center gap-2 relative ${itemVisuallyDone ? 'bg-[#f0eef5]/80 border-[rgba(115,102,148,0.2)]' : `bg-white/70 ${isCottagecore ? 'cc-border' : 'border-white/85'}`}`}>
              {isCottagecore && (() => {
                ...
                if (d===2) return <><Mushroom  width={15} rotate={-20} style={{...s, right:2,    top:-8 }} /><LeafBig   width={14} rotate={50}  style={{...s, left:'60%', bottom:-7}} /><Flower    width={12} rotate={40}  style={{...s, left:4,    top:-6 }} /></>
                if (d===3) return <><LeafBig   width={16} rotate={-35} style={{...s, right:0,    bottom:-8}} /><Flower    width={13} rotate={-15} style={{...s, left:'35%', bottom:-6}} /><LeafSmall width={13} rotate={70}  style={{...s, left:2,    top:-7 }} /></>
                if (d===4) return <><Mushroom  width={14} rotate={25}  style={{...s, right:8,    top:-8 }} /><LeafSmall width={13} rotate={-55} style={{...s, left:'48%', bottom:-6}} /><Flower    width={14} rotate={-30} style={{...s, left:6,    bottom:-7}} /></>
                if (d===5) return <><LeafBig   width={13} rotate={40}  style={{...s, right:0,    top:-7 }} /><Flower    width={14} rotate={60}  style={{...s, left:'25%', top:-6 }} /><Mushroom  width={15} rotate={-5}  style={{...s, right:10,  bottom:-8}} /></>
        ...
        <div className="flex flex-col gap-3 overflow-y-auto flex-1">
          ...
          {viewMode === 'checklist' && (
            <>
              ...
              {(() => {
                return <>
                  {renderCkGrouped(checklistItems)}
