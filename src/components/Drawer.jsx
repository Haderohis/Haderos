...
const NAV_ITEMS = [
  {
    ...
    label: 'Home',
    icon: (active) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill={active ? 'white' : 'rgb(var(--color-muted))'}>
...
export default function Drawer({ open, onClose }) {
  ...
  return (
    <>
      ...
      <div
        onClick={onClose}
        className={`fixed inset-0 bg-dark/[0.18] z-30 transition-opacity duration-300 ${
      ...
      {/* Drawer */}
      <nav
        className={`fixed top-0 left-0 h-full w-[280px] z-40 bg-base transition-transform duration-300 ${
      ...
      >
        ...
        <div className="absolute inset-0 bg-white/72 border-r border-white/85 backdrop-blur-md flex flex-col">
          ...
          <div className="flex items-center gap-4 px-4 pt-16 pb-5">
            <div className="w-[52px] h-[52px] rounded-[26px] bg-primary flex items-center justify-center shrink-0">
              <span className="text-white text-[20px] font-bold">{initials}</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[15px] font-bold text-dark leading-tight truncate">{firstName} {lastName}</p>
              <p className="text-[12px] text-muted leading-tight truncate">{displayName}</p>
            </div>
            <button onClick={() => { onClose(); navigate('/settings') }} className="shrink-0 w-6 h-6 flex items-center justify-center opacity-50">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="rgb(var(--color-muted))">
            ...
            {NAV_ITEMS.map(({ path, label, icon }) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-3 w-full h-[52px] px-3 rounded-[12px] shrink-0 ${active ? 'bg-soft' : ''}`}
                >
                  <div className={`w-7 h-7 rounded-[8px] flex items-center justify-center shrink-0 overflow-hidden ${active ? 'bg-primary' : 'bg-soft'}`}>
                    {icon(active)}
                  </div>
                  <span className={`text-[14px] flex-1 text-left ${active ? 'font-semibold text-primary' : 'font-normal text-muted/85'}`}>
                    {label}
                  </span>
          ...
          <div className="px-4 pb-10 pt-3 shrink-0 border-t border-[rgba(153,153,166,0.15)]">
            <button
              ...
              className="flex items-center justify-center gap-2 w-full h-[46px] border border-muted rounded-[8px]"
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="rgb(var(--color-muted))">
                <path d="M17 7l-1.41 1.41L18.17 11H8v2h10.17l-2.58 2.58L17 17l5-5-5-5zM4 5h8V3H4c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h8v-2H4V5z" />
              </svg>
              <span className="text-[12px] font-semibold text-muted">Se déconnecter</span>
            </button>
