...
export function FieldLabel({ children, required }) {
  return (
    <label className="text-[12px] font-medium text-[#736694]">
      {children}{required && <span className="text-[#6c63ff] ml-0.5">*</span>}
    </label>
  )
}
        ...
        {options.map(opt => (
          <button
            ...
            className={`flex-1 rounded-[8px] text-[13px] font-semibold transition-colors ${
              value === opt.value ? 'bg-[#6c63ff] text-white' : 'text-[#736694]'
            }`}
          >
            {opt.label}
          </button>
...
export function SubmitButton({ children, disabled, onClick }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="bg-[#6c63ff] rounded-[12px] h-12 text-[14px] font-semibold text-white disabled:opacity-40 transition-opacity"
    >
      {children}
    </button>
  )
